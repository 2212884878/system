			...
			<el-col :span='24' class="tabs">
				<el-table :data="data" border highlight-current-row height="500" :header-cell-style="tableHeaderColor" v-loading="loading" element-loading-text="拼命加载数据中"
				 element-loading-spinner="el-icon-loading" style="width: 100%" size="mini" ref="multipleTable" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="createDate" label="创建日期" min-width="150" align="center"></el-table-column>
					<el-table-column prop="bs" label="PC网页" align="center" min-width="100">
						<template slot-scope="scope">
							<template v-if="scope.row.bs == 1">
					...
					<el-table-column prop="app" label="APP" min-width="100" align="center">
						<template slot-scope="scope">
							<template v-if="scope.row.app == 1">
								<el-tag size="medium" type="success">是</el-tag>
							</template>
							<template v-else>
								<el-tag size="medium" type="danger">否</el-tag>
					...
					<el-table-column prop="weiXin" min-width="100" label="微信公众号" align="center">
						<template slot-scope="scope">
							<template v-if="scope.row.weiXin == 1">
								<el-tag size="medium" type="success">是</el-tag>
							</template>
							<template v-else>
								<el-tag size="medium" type="danger">否</el-tag>
					...
					<el-table-column prop="samllProgress" min-width="100" label="微信小程序" align="center">
						<template slot-scope="scope">
							<template v-if="scope.row.samllProgress == 1">
								<el-tag size="medium" type="success">是</el-tag>
							</template>
							<template v-else>
								<el-tag size="medium" type="danger">否</el-tag>
